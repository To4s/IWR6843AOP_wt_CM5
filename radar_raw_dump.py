...
#gracefully stop scheduler and close MAVLink connection
def shutdown_handler(signum, frame):
    print("\nShutting down gracefully...")
    sched.shutdown(wait=False)  # Stop the scheduler
    conn.close()                # Close MAVLink connection
...
def wait_for_pixhawk_heartbeat(timeout=30):
    ...
    while time.time() - start < timeout:
        ...
        if msg:
            print("Received heartbeat from Pixhawk")
            return True
        # Retry heartbeat
        conn.mav.heartbeat_send(
            mavutil.mavlink.MAV_TYPE_ONBOARD_CONTROLLER,
    ...
    print("No response from Pixhawk.")
    return False
...
pixhawk_connected = wait_for_pixhawk_heartbeat()
...
if pixhawk_connected:
    ...
    print('Unable to connect to Pixhawk')
    sys.exit(0)
...
configFileName = 'testconfig1.cfg'
CLIport = {}
Dataport = {}
